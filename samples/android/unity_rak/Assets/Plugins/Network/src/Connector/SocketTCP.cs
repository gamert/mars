...
namespace Comm.Network
{
    ...
    class TcpSocketImp : SocketBase
    {
        ...
        public override int connectServer(string ip, int port, BaseConnector mgr)
        {
            ...
            {
                LogError(string.Format("connectServer: 已经连接:{0},{1} ==> {2},{3}", tcpServerIp, tcpPort, ip,port));
                if (tcpServerIp == ip && tcpPort == port)
                    return 0;
                else
                    this.Close();
            }
            ...
            try
            {
                ...
                BaseConnector.Log("Create Socket : " + tcpServerIp + "," + tcpPort+";"+ ipAddress.AddressFamily);
                ...
                //clientSock.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.SendTimeout, 5000);
                //服务器端口
        ...
        //at Comm.Network.TcpSocketImp.SetKeepALive (System.Net.Sockets.Socket socket) [0x00065] 
        //
        ...
        protected void ProcessConnect(Socket socket, object state, SocketAsyncEventArgs e)
        {
            ...
            {
                LogError(string.Format("ProcessConnect 0:,{0}", e.SocketError));
                OnTimeOut();
                return;
            }
            ...
            if (socket == null)
            {
                LogError(string.Format("ProcessConnect 1:,{0}", e.SocketError));
                OnTimeOut();
                return;
            }
            if (e == null)
        ...
        void StartReceive()
        {
            ...
            catch (Exception e)
            {
                ...
                {
                    BaseConnector.Log(string.Format("StartReceive0:{0}:{1}",this.tcpPort, e.Message));
                }
                else if (e is NullReferenceException)
                {
                    BaseConnector.Log(string.Format("StartReceive1:{0}:{1}", this.tcpPort, e.Message));
                }
                else
                {
                    //#if !WIN_CLIENT
                ...
                {
                    BaseConnector.Log(string.Format("StartReceive2:{0}:{1}", this.tcpPort, e2.Message));
                }
                return;
            }
        ...
        private void ProcessReceive(SocketAsyncEventArgs e)
        {
            ...
            if (e.SocketError != SocketError.Success)
            {
                ...
                LogError("ProcessReceive:2 SocketException=" + e.SocketError.ToString() );
                m_Status = SOCKET_STATUS.CLOSING;
                OnTimeOut();
                return;
            }
            ...
            //BaseConnector.Log(string.Format("OnDataReceived{0} {1} {2}", e.Buffer, e.Offset, e.BytesTransferred));
        ...
        public override bool Send(byte[] buffer, int offset, int size)
        {
            ...
            if (_clientSock == null)
            {
                BaseConnector.Log(string.Format("SocketTCP::Send:_clientSock{0} == null",this.tcpPort));
                return false;
            }
            if (_clientSock.Connected == false)
            {
                BaseConnector.Log(string.Format("SocketTCP::Send:_clientSock{0}.Connected == false", this.tcpPort));
                Dispose();
                return false;
            }
            //BaseStructRev pp = BaseStructRevPool.SafeCreate(buffer, offset, size);
        ...
        private void sendCallback(IAsyncResult asyncSend)
        {
            ...
            if (bytesSent == 0)
            {
                LogError(string.Format("Socket{0} Send err bytesSent == 0!", this.tcpPort));
                //if (e.SocketError != SocketError.WouldBlock)
                {
                    Dispose();
                }
            ...
            //{
            //    UnityEngine.Debug.LogWarning("Send: "+ mNeedSendMsg.Count);
            //    //SendTcpMessage(mNeedSendMsg[0]);
