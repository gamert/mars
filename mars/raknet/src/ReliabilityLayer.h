...
public:
	...
	void Preallocate(InternalPacket * internalPacket, const char *file, unsigned int line)
	{
		...
		for (int i = 0; i < (int)allocation_size; ++i)
		{
			data[i] = NULL;
		}
	}
...
public:
	...
	//zzTest:
	void init_test(DatagramSequenceNumberType index);
	void set_receivedPacketsBaseIndex(DatagramSequenceNumberType index);
private:
...
//	void DeleteSequencedPacketsInList( unsigned char orderingChannel, DataStructures::Queue<InternalPacket*>&theList );
	...
	//zzAdd #1625 SIGSEGV(SEGV_MAPERR)
	SimpleMutex unreliableWithAckReceiptHistory_lock;
