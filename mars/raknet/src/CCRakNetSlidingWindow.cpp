...
#else
static const CCTimeType SYN=10000;	//zz: 用于控制ACK发送延时
#endif
...
void CCRakNetSlidingWindow::Init(CCTimeType curTime, uint32_t maxDatagramPayload)
{
    ...
    if(maxDatagramPayload>MAXIMUM_MTU_SIZE)
        maxDatagramPayload=MAXIMUM_MTU_SIZE;
	MAXIMUM_MTU_INCLUDING_UDP_HEADER=maxDatagramPayload;	//1492-28 = 1464
...
void CCRakNetSlidingWindow::OnResend(CCTimeType curTime, RakNet::TimeUS nextActionTime)
{
	...
	if (_isContinuousSend && backoffThisBlock==false && cwnd>MAXIMUM_MTU_INCLUDING_UDP_HEADER*2)
	{
		...
		// CC PRINTF
#ifdef RAKNET_LOG_ZZ
		RAKNET_LOG("-- %.0f (Resend) Enter slow start.\n", cwnd);
#endif
	}
}
// ----------------------------------------------------------------------------------------------------------------------------
...
void CCRakNetSlidingWindow::OnAck(CCTimeType curTime, CCTimeType rtt, bool hasBAndAS, BytesPerMicrosecond _B, BytesPerMicrosecond _AS, double totalUserDataBytesAcked, bool isContinuousSend, DatagramSequenceNumberType sequenceNumber )
{
	...
	else
	{
		...
		estimatedRTT = estimatedRTT + d * difference;	//zz: 趋近一般的差值
	...
	if (IsInSlowStart())
	{
		...
		// CC PRINTF
#ifdef CC_PRINTF
		printf("++ %.0lf Slow start increase.\n", cwnd);
#endif
	...
	else if (isNewCongestionControlPeriod)
	{
		...
		// CC PRINTF
#ifdef CC_PRINTF
		 printf("+ %.0lf Congestion avoidance increase.\n", cwnd);
#endif
	}

#ifdef RAKNET_LOG_ZZ
	RAKNET_LOG("OnAck(cwnd=%lf-ssThresh=%lf)%d.\n", cwnd, ssThresh, isNewCongestionControlPeriod);
#endif	
...
#else
	const CCTimeType maxThreshold=2000000;	//最大2s
	//const CCTimeType minThreshold=100000;
//	const CCTimeType additionalVariance=30000;
	const CCTimeType additionalVariance=30000;  //我考虑使用累进
    if (timesSent < 1)timesSent = 1;
#endif
...
//	CCTimeType threshhold = (CCTimeType) (u * estimatedRTT + q * deviationRtt) + additionalVariance ;
	CCTimeType threshhold = (CCTimeType) (u * estimatedRTT + q * deviationRtt) + additionalVariance*(1 << (timesSent-1));
