...
public:
	...
	ConnectionAttemptResult Reconnect(RakNet::TimeMS timeoutTime);
	...
	/// \brief Gets a bunch of messages from the incoming message queue.
	unsigned int Receive(Packet **ppPacketList, unsigned int maxSize);
...
//		int connectionSocketIndex; // index into connectionSockets to send back on.
	...
	void Reset(const AddressOrGUID& systemIdentifier);
#endif
...
protected:
	...
	//处理协议：
	friend RakPeer::RemoteSystemStruct *Process_ID_OPEN_CONNECTION_REPLY_2(int cmd, SystemAddress &systemAddress, RakPeer *rakPeer, RakNetSocket2* rakNetSocket, RakNetGUID &guid, SystemAddress &bindingAddress, uint16_t mtu, bool doSecurity, RakNet::TimeUS timeRead);
