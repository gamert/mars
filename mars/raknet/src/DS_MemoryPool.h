...
namespace DataStructures
{
	...
	void MemoryPool<MemoryBlockType>::Release(MemoryBlockType *m, const char *file, unsigned int line)
	{
		...
		if (!curPage){CCLOG(2,"MemoryPool::Release(%x) null curPage\n", (size_t)m);return;}	//zzAdd:
		if (curPage->availableStackSize==0)	//#215 SIGSEGV (DS_MemoryPool.h:165)
		{
		...
		else
		{
			...
			if (curPage->availableStackSize >= bpp) { CCLOG(2,"MemoryPool::Release(curPage->availableStackSize=%d)\n", curPage->availableStackSize); return; }	//zzAdd: 
			curPage->availableStack[curPage->availableStackSize++]=memoryWithPage;
