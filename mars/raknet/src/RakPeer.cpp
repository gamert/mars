...
//zzModi: 
static const int NUM_MTU_SIZES=2;
static const int mtuSizes[NUM_MTU_SIZES]={MAXIMUM_MTU_SIZE, 576 };//, 1200, 576
...
// 	return p;
	RakNet::Packet *p = AllocPacket(dataSize,NULL, file, line);
...
Packet *RakPeer::AllocPacket(unsigned dataSize, unsigned char *data, const char *file, unsigned int line)
{
	...
	p->length = dataSize;
//#if defined(_WIN32)
//	p->length = o > 0 && rand() % 1000 > max(1000 - (n++ / 300) * 10, 1) ? 1 : dataSize;
//#else
//	p->length = o > 0 && rand() % 1000 > fmax(1000 - (n++ / 300) * 10, 1) ? 1 : dataSize;
//#endif
...
//	}
	...
	p->bitSize=BYTES_TO_BITS(p->length);
	p->deleteData=true;
	p->guid=UNASSIGNED_RAKNET_GUID;
...
void RakPeer::DeallocatePacket(Packet *packet)
{
	...
	if (packet->deleteData )//&& packet->guard == 'DAUG'
	{
		if (packet->data)
		{
	...
	{
		//CCLOG(2,"RakPeer::DeallocatePacket: packet=%x,guard=%x\n", (size_t)packet, packet->guard);
...
ConnectionAttemptResult RakPeer::Connect( const char* host, unsigned short remotePort, const char *passwordData, int passwordDataLength, PublicKey *publicKey, unsigned connectionSocketIndex, unsigned sendConnectionAttemptCount, unsigned timeBetweenSendConnectionAttemptsMS, RakNet::TimeMS timeoutTime )
{
	...
	{
		xerror2(TSF"RakPeer::Connect: INVALID_PARAMETER,connectionSocketIndex=%_\n", connectionSocketIndex);
...
//		return false;
	...
	ConnectionAttemptResult res = SendConnectionRequest( host, remotePort, passwordData, passwordDataLength, publicKey, connectionSocketIndex, 0, sendConnectionAttemptCount, timeBetweenSendConnectionAttemptsMS, timeoutTime);
	xinfo2(TSF"res = %_ \n", res);
	return res;
}

//重连:
ConnectionAttemptResult RakPeer::Reconnect(RakNet::TimeMS timeoutTime)
{
	//1. 需要关闭吗？
...
#endif
unsigned int RakPeer::Receive(Packet **ppPacketList, unsigned int maxSize)
{
	if (ppPacketList == (Packet**)0)
	{
		//ppPacketList = (Packet**)rakMalloc_Ex(sizeof(Packet*) * maxSize, _FILE_AND_LINE_);
		//memset(ppPacketList, 0, sizeof(Packet*) * maxSize);
		//Kaede: not allocate memory within peer.
		return 0;
	}
	...
	//DataStructures::LinkedList<Packet*> rPacketLst;
	//rPacketLst.End();

	i = 0;
	packetReturnMutex.Lock();
	while (!packetReturnQueue.IsEmpty() && amount < maxSize)
	{
		Packet* packet = packetReturnQueue.Pop();
	//	if (packet != (Packet*)0)
	//	{
	//		xinfo2(TSF"Receive[Ord%_,Seq%_] Reli=%_;local_id=%_;Len=%_\n", (int)packet->orderingIndex, (int)packet->sequencingIndex, packet->reliability, packet->local_id, packet->bitSize);

	//		rPacketLst.Add(packet);
	//		++i;
	//	}		
	//}
	//packetReturnMutex.Unlock();
	...
	//unsigned int j = 0;
	//rPacketLst.Beginning();
	//while (j < rPacketLst.Size())
	//{
		//Packet* packet = rPacketLst.Peek();
		//++rPacketLst;
		//++j;
		...
		//		packet = (Packet*)0; // Will do the loop again and get another packet
		...
		//for (i = 0; packet != (Packet*)0 && i < pluginListNTS.Size(); i++)
		//{
		...
		{
#if RAKNET_DEBUG_PAK_SEQ
			xinfo2(TSF"Receive end [Ord%_,Seq%_] Reli=%_;local_id=%_;Len=%_\n", (int)packet->orderingIndex, (int)packet->sequencingIndex, packet->reliability, packet->local_id, packet->bitSize);
#endif
			ppPacketList[amount++] = packet;
		}
	}
	packetReturnMutex.Unlock();
...
void RakPeer::CloseConnection( const AddressOrGUID target, bool sendDisconnectionNotification, unsigned char orderingChannel, PacketPriority disconnectionNotificationPriority )
{
	...
	*/
	...
	xinfo2(TSF":target=%_;sendDisconnectionNotification=%_\n", target.ToString(), sendDisconnectionNotification);
	...
	ConnectionState cs = GetConnectionState(target);
	if (sendDisconnectionNotification==false)
	{
		xinfo2(TSF":GetConnectionState =%_;\n", cs);
		if (cs == IS_CONNECTED)
		{
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#if _MODULE_FIX_CONNECTION_ATTEMPT == 1
...
ConnectionState RakPeer::GetConnectionState(const AddressOrGUID systemIdentifier)
{
	...
	if (index == -1)
	{
		//xerror2(TSF"GetConnectionState(%_) index = %_ ,IS_NOT_CONNECTED\n", systemIdentifier.ToString(), index);
		return IS_NOT_CONNECTED;
	}

	if (remoteSystemList[index].isActive == false)
	{
		xerror2(TSF"GetConnectionState(%_) remoteSystemList[%_].isActive == false ,IS_DISCONNECTED\n",systemIdentifier.ToString(), index);
...
void RakPeer::InsertPacket(Packet* pkt)
{
	sendSeqMutex.Lock();	
	unsigned char c = pkt->data[0];
	if (c == 0x8c) sendSeq += 100000;
	if (c == 0x8d) sendSeq = 0;
	sendSeqMutex.Unlock();

	Packet* packet = AllocPacket(pkt->length, pkt->data, _FILE_AND_LINE_);	
	packet->systemAddress = pkt->systemAddress;
	packet->systemAddress.systemIndex = pkt->systemAddress.systemIndex;
	packet->guid = pkt->guid;
	packet->guid.systemIndex = pkt->guid.systemIndex;
...
#endif
//@sendConnectionAttemptCount: 默认连接次数6
//@timeBetweenSendConnectionAttemptsMS:间隔1s
//@timeoutTime:超时时间
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
ConnectionAttemptResult RakPeer::SendConnectionRequest( const char* host, unsigned short remotePort, const char *passwordData, int passwordDataLength, PublicKey *publicKey, unsigned connectionSocketIndex, unsigned int extraData, unsigned sendConnectionAttemptCount, unsigned timeBetweenSendConnectionAttemptsMS, RakNet::TimeMS timeoutTime )
{
	...
	{
		xerror2(TSF"CANNOT_RESOLVE_DOMAIN_NAME");
	...
	{
		xerror2(TSF"ALREADY_CONNECTED_TO_ENDPOINT");
	...
	rcs->nextRequestTime=RakNet::GetTimeMS();	//zz:下1Frame，立即发送请求
	rcs->requestsMade=0;	//请求次数
	rcs->data=0;
	rcs->socket=0;
	...
	for (; i < requestedConnectionQueue.Size(); i++)
	{
		if (requestedConnectionQueue[i]->systemAddress==systemAddress)
		{
			...
			xerror2(TSF"CONNECTION_ATTEMPT_ALREADY_IN_PROGRESS");
	...
	xinfo2(TSF"CONNECTION_ATTEMPT_STARTED");
...
#ifdef RAKNET_LOG_ZZ
	RAKNET_LOG("RakPeer::PingInternal target=%s;performImmediate=%d\n", target.ToString(), performImmediate);
#endif
...
{
	xinfo2(TSF"systemIdentifier=%_;sendDisconnectionNotification=%_;performImmediate=%_;remoteSystemList=%_;endThreads=%_\n", systemIdentifier.ToString(), sendDisconnectionNotification, performImmediate, remoteSystemList, endThreads);
	...
	else
	{
		if (performImmediate)
		{
			...
			{
				xinfo2(TSF"GetRemoteSystemIndex()=%_; or !isActive", index);
			}
		}
		else
		{
...
namespace RakNet {
	...
	RakPeer::RemoteSystemStruct *Process_ID_OPEN_CONNECTION_REPLY_2(int cmd, SystemAddress &systemAddress,
		RakPeer *rakPeer, 
		RakNetSocket2* rakNetSocket, 
		RakNetGUID &guid, 
		SystemAddress &bindingAddress, 
		uint16_t mtu, bool doSecurity, RakNet::TimeUS timeRead)
	{
			...
			if (rcs->systemAddress == systemAddress)
			{
				...
				if (thisIPConnectedRecently == false)
				{
					...
					if (remoteSystem)
					{
						...
						if (cmd == ID_OPEN_CONNECTION_REPLY_2)
						{
...
/*
	...
	if (*isOfflineMessage)
	{
		xinfo2(TSF"isOfflineMessage : cmd=%_\n", (int)data[0]);
		...
		else if ((unsigned char)(data)[0] == (MessageID)ID_OPEN_CONNECTION_REPLY_2)
		{
			...
			Process_ID_OPEN_CONNECTION_REPLY_2(ID_OPEN_CONNECTION_REPLY_2, systemAddress, rakPeer, rakNetSocket, guid, bindingAddress, mtu, doSecurity,timeRead);
			...
			(unsigned char)(data)[0] == (MessageID)ID_INCOMPATIBLE_PROTOCOL_VERSION)
		{
			...
			if ((unsigned char)(data)[0] == (MessageID)ID_ALREADY_CONNECTED)
			{
				...
				uint16_t mtu = MAXIMUM_MTU_SIZE;
				bool doSecurity = false;
				bool b = bs.Read(bindingAddress); externalID = bindingAddress;
				b = bs.Read(mtu);
				b = bs.Read(doSecurity);

				remoteSystem = Process_ID_OPEN_CONNECTION_REPLY_2(ID_ALREADY_CONNECTED,systemAddress, rakPeer, rakNetSocket, guid, bindingAddress, mtu, doSecurity, timeRead);
				if (remoteSystem != NULL)
				{
					...
					bs.Read(systemIndex);
					for (unsigned int i = 0; i < MAXIMUM_NUMBER_OF_INTERNAL_IDS; i++)
						bs.Read(remoteSystem->theirInternalSystemAddress[i]);

					remoteSystem->myExternalSystemAddress = externalID;	//
					remoteSystem->connectMode = RakPeer::RemoteSystemStruct::CONNECTED;
				
					connectionAttemptCancelled = true;
				}
				else
				{
					xinfo2(TSF"rebuild fail: %_", externalID.ToString());
				}
			}
#endif
			//处理其他类型...
			//if (remoteSystem == NULL)
			{
				...
				for (i = 0; i < rakPeer->requestedConnectionQueue.Size(); i++)
				{
						...
						&& rcs->systemAddress == systemAddress)
					{
						...
						xinfo2(TSF"requestedConnectionQueue RemoveAtIndex %_", systemAddress.ToString());
#if LIBCAT_SECURITY==1
...
#	endif
		...
		else if ((unsigned char)(data)[0] == ID_OPEN_CONNECTION_REQUEST_2)
		{
			...
			else if (outcome!=0)
			{
				...
				bsOut.Write(mtu);
				bsOut.Write(requiresSecurityOfThisClient);
			
				SystemIndex systemIndex = (SystemIndex)rakPeer->GetIndexFromSystemAddress(systemAddress, true);
				bsOut.Write(systemIndex);
				for (unsigned int i = 0; i < MAXIMUM_NUMBER_OF_INTERNAL_IDS; i++)
					bsOut.Write(rakPeer->ipList[i]);
...
{
//#ifdef RAKNET_LOG_ZZ
//    xinfo2(TSF"\n[%_]ProcessNetworkPacket systemAddress=%_;length=%_ ;\n", timeRead, systemAddress.GetPort(),length);
//#endif
...
*/
#if RAKNET_DEBUG_PAK_SEQ
#define DEBUG_SET_SEQ(packet) \
	packet->orderingIndex = orderingIndex;\
	packet->sequencingIndex = sequencingIndex;\
	packet->reliability = reliability;\
	packet->local_id = local_id;
#else
#define DEBUG_SET_SEQ(packet) 
#endif
...
//	unsigned int socketListIndex;
	...
	while ((bcs=bufferedCommands.PopInaccurate())!=0)
	{
		...
		if (bcs->command != 0)
		{
			xinfo2(TSF"[%_]RakPeer::RunUpdateCycle bcs->command =%_\n", timeMS, bcs->command);
		}
#endif
		...
		while (requestedConnectionQueueIndex < requestedConnectionQueue.Size())
		{
			...
			if (rcs->nextRequestTime < timeMS)
			{
				...
				else
				{
					...
					// Kaede: crash if rcs->sendConnectionAttemptCount < NUM_MTU_SIZES: divided by zero
					//int MTUSizeIndex = rcs->requestsMade / (rcs->sendConnectionAttemptCount/NUM_MTU_SIZES);
					int MTUSizeIndex = 0;					
					if (rcs->sendConnectionAttemptCount < NUM_MTU_SIZES)
						MTUSizeIndex = (rcs->requestsMade + 1) % NUM_MTU_SIZES;
					else
...
//					SocketLayer::SetDoNotFragment(socketToUse, 1);
	...
	//	remoteSystemList[ remoteSystemIndex ].allowSystemAddressAssigment=true;
			...
			if (timeMS > remoteSystem->lastReliableSend && timeMS-remoteSystem->lastReliableSend > remoteSystem->reliabilityLayer.GetTimeoutTime()/2 && remoteSystem->connectMode==RemoteSystemStruct::CONNECTED)
			{
				...
				if (remoteSystem->reliabilityLayer.GetResendListDataSize()==0)
				{
			...
			{
				xinfo2(TSF"timeMS=%_ remoteSystem->connectMode =%_, connectionTime=%_\n", timeMS, remoteSystem->connectMode,remoteSystem->connectionTime );
...
//					memcpy(packet->data+1, undeliveredMessages.GetData(), undeliveredMessages.GetNumberOfBytesUsed());
...
#if RAKNET_DEBUG_PAK_SEQ
				, orderingIndex, sequencingIndex, reliability, local_id
#endif
				);
			...
			while ( bitSize > 0 )
			{
				...
				if (remoteSystem->connectMode==RemoteSystemStruct::UNVERIFIED_SENDER)
				{
					...
					{
						CloseConnectionInternal( systemAddress, false, true, 0, LOW_PRIORITY );
						xinfo2(TSF"Temporarily banning %_ for sending nonsense data\n", systemAddress.ToString());
					...
					else
					{
						...
						{						
							...
							if (data[0] == 0x8c || data[0] == 0x8d)
								BroadcastPacket2AllPeers(this, packet);
						} 
						else
						{
							rakFree_Ex(data, _FILE_AND_LINE_ );
...
void RakPeer::Reset(const AddressOrGUID& systemIdentifier)
{
	if (systemIdentifier.IsUndefined())
		return;
	...
	if (index != (unsigned int)-1)
	{
		remoteSystemList[index].reliabilityLayer.Reset(true, remoteSystemList[index].MTUSize, false);
	}	
}
#endif
