...
public:
	virtual unsigned int Receive(Packet** ppPacketList, unsigned int maxSize){return 0;};
#if _MODULE_KEEP_CONN_AND_RESET == 1
	virtual void Reset(const AddressOrGUID& systemIdentifier) {};
#endif
	...
	//zzAdd: for reconnect:
	virtual ConnectionAttemptResult Reconnect(RakNet::TimeMS timeoutTime = 0) = 0;
    ...
    // dumb
    virtual void InsertPacket(Packet* pkt) = 0;
