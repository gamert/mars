...
void BPSTracker::ClearExpired1(RakNet::TimeUS time)
{
		...
		)
	{
	 ...
	 BitSize_t DatagramHeaderFormat::GetDataHeaderBitLength()
	{
		return BYTES_TO_BITS(GetDataHeaderByteLength());
	}

	 unsigned int  DatagramHeaderFormat::GetDataHeaderByteLength()
	{
	 ...
	 void  DatagramHeaderFormat::Serialize(RakNet::BitStream *b)
	{
		// Not endian safe
	 ...
	 void  DatagramHeaderFormat::Deserialize(RakNet::BitStream *b)
	{
		// Not endian safe
...
#if RAKNET_LOG_ZZ==1
	 void DatagramHeaderFormat::Dump()
	 {
		 RAKNET_LOG("       isValid=%d,isACK=%d,", isValid, isACK);
		 if (isACK)
		 {
			 RAKNET_LOG("hasBAndAS=%d,AS=%f;\n", hasBAndAS, AS);
		 }
		 else
		 {
			 if (isNAK)
				 ...
				 RAKNET_LOG("isPacketPair=%d,isContinuousSend=%d,needsBAndAs=%d,datagramNumber=%d\n", isPacketPair, isContinuousSend, needsBAndAs, (int)datagramNumber);
		 }
	 }
#endif
...
inline void CalcStatistics(RakNetStatistics *rns)
{
...
inline void outputQueue_Push(DataStructures::Queue<InternalPacket*> &outputQueue, InternalPacket *internalPacket, const char *file, unsigned int line)
{
	...
	internalPacket->local_id = s_local_id++;
	xinfo2(TSF"internalPacket[Ord%_,Seq%_] Reli=%_;local_id=%_;Len=%_\n", (int)internalPacket->orderingIndex, (int)internalPacket->sequencingIndex, internalPacket->reliability, internalPacket->local_id,internalPacket->dataBitLength);
#endif
	outputQueue.Push(internalPacket, file, line);
}
...
void ReliabilityLayer::doStatistics()
{
	...
	if(statistics.runningTotal[ACTUAL_BYTES_RECEIVED])
	{
		...
		CalcStatistics(&statistics);
		StatisticsToString(&statistics, text, 3);
		if (text[0])
		{
...
#endif
	unreliableWithAckReceiptHistory_lock.Lock();
...
//	RAKNET_LOG("[%d]ReliabilityLayer::HandleSocketReceiveFromConnectedPlayer:(length=%d)\n", (unsigned int)(timeRead / 1000), length);
...
#endif
//#ifdef RAKNET_LOG_ZZ
//	RAKNET_LOG("ReliabilityLayer::HandleSocketReceiveFromConnectedPlayer(length=%d)\n", length);
//#endif
	...
	if (dhf.isACK)
	{
		...
		//	RAKNET_LOG("%i ", (RakNet::TimeMS)(rtt/1000));
		ackPing=rtt;
#endif
		...
		for (i=0; i<incomingAcks.ranges.Size();i++)
		{
			...
			uint32_t minIndex = (uint32_t)incomingAcks.ranges[i].minIndex;
			uint32_t maxIndex = (uint32_t)incomingAcks.ranges[i].maxIndex;

#ifdef RAKNET_LOG_ZZ
			RAKNET_LOG("incomingAcks [%d] ACK min maxIndex[%u-%u]\n",i,minIndex, maxIndex);
#endif		

#if _MODULE_FIX_ACK_NAK_DOS == 1
            int32_t gap = ((int)maxIndex) - ((int)minIndex);
			if (gap > acks_limit_threshold)
				maxIndex = minIndex + acks_limit_threshold;
#endif			
			for (datagramNumber = minIndex; datagramNumber >= minIndex && datagramNumber <= maxIndex; datagramNumber++)
			{
				CCTimeType whenSent;
				...
				if (unreliableWithAckReceiptHistory.Size()>0)
				{
					...
					while (k < unreliableWithAckReceiptHistory.Size())
					{
						if (unreliableWithAckReceiptHistory[k].datagramNumber == datagramNumber)
						{
							...
							outputQueue_Push(outputQueue, ackReceipt, _FILE_AND_LINE_ );
				...
				//根据datagramNumber找到可靠消息节点...对于非reliable报文，这里是不会有任何的ack发生。。。
...
#else
					CCTimeType ping;
					if (timeRead>whenSent)	//zz: 这里计算一次RTT
						ping=timeRead-whenSent;
					else
						ping=0;

					//totalUserDataBytesAcked: 总确认字节?
					//isContinuousSend: bandwidthExceededStatistic ,这个在update里面设置，相当于，如果 没有发送完全，则认为是连续发送...
...
// 					RAKNET_LOG("%p Ack %i is duplicate\n", this, datagramNumber.val);
// 
//  					congestionManager.OnDuplicateAck(timeRead, datagramNumber);
// 				}
			}
		}//end of incomingAcks.ranges.Size()
	}
	else if (dhf.isNAK)
	{
...
#ifdef RAKNET_LOG_ZZ
			//			RAKNET_LOG("		[%d] NACK[%d-%d]\n",i, incomingNAKs.ranges[i].minIndex, incomingNAKs.ranges[i].maxIndex);
#endif		
			// Sanity check
			//RakAssert(incomingNAKs.ranges[i].maxIndex.val-incomingNAKs.ranges[i].minIndex.val<1000);
			uint32_t minIndex = (uint32_t)incomingNAKs.ranges[i].minIndex;
			uint32_t maxIndex = (uint32_t)incomingNAKs.ranges[i].maxIndex;	//
#if _MODULE_FIX_ACK_NAK_DOS == 1
            int32_t gap = ((int)maxIndex) - ((int)minIndex);
...
#ifdef RAKNET_LOG_ZZ
			xerror2(TSF"ERROR: CreateInternalPacketFromBitStream: fail,dhf.datagramNumber=%_\n", (int)dhf.datagramNumber);
#endif
			return true;
		}
...
#else
			//if (internalPacket->reliableMessageNumber.val == 10)
			//{
			//	RAKNET_LOG("TEST: internalPacket->reliableMessageNumber.val == 10,ignore\n");
			//}
			//else
			{
		...
		while ( internalPacket )
		{
...

			{
				...
				if (internalPacket->reliability == RELIABLE || internalPacket->reliability == RELIABLE_SEQUENCED || internalPacket->reliability == RELIABLE_ORDERED )
				{
					...
					// TESTING1
#ifdef RAKNET_LOG_ZZ
//					RAKNET_LOG("waiting on reliableMessageNumber=%i holeCount=%i[%i] datagramNumber=%i\n", receivedPacketsBaseIndex.val, holeCount.val, internalPacket->reliableMessageNumber, dhf.datagramNumber.val);
...
CONTINUE_SOCKET_DATA_PARSE_LOOP:
	...
	return true;
}//end of HandleSocketReceiveFromConnectedPlayer
	...
	)
{
	...
	if ( outputQueue.Size() > 0 )
	{
		...
		*data = internalPacket->data;
		...
		sequencingIndex = internalPacket->sequencingIndex;
		reliability = internalPacket->reliability;
		local_id = internalPacket->local_id;
#endif
		ReleaseToInternalPacketPool( internalPacket );
		return bitLength;
	}
...
bool ReliabilityLayer::Send( char *data, BitSize_t numberOfBitsToSend, PacketPriority priority, PacketReliability reliability, unsigned char orderingChannel, bool makeDataCopy, int MTUSize, CCTimeType currentTime, uint32_t receipt )
{
	...
	//if (g_log == 2)
	//{
	//	RAKNET_LOG("[%d]ReliabilityLayer:Send(numberOfBitsToSend=%d)\n", (unsigned int)(currentTime / 1000), numberOfBitsToSend);
	//}
...
//#endif
						...
						RemoveFromUnreliableLinkedList(cur);		//仅仅处理不可靠报文...				
	...
	// 如果超过10s没有收到报文，设置 deadConnection
...
#endif
	{
		xinfo2(TSF"deadConnection: statistics.messagesInResendBuffer=%_\n", statistics.messagesInResendBuffer);
	...
	dhf.isContinuousSend=bandwidthExceededStatistic;			//如果前一次没有发送完全?
...
#ifdef RAKNET_LOG_ZZ
//		RAKNET_LOG("	transmissionBandwidth:%d retransmissionBandwidth:%d IsResendQueueEmpty()=%d\n", transmissionBandwidth, retransmissionBandwidth, IsResendQueueEmpty());
#endif
...
//                    RAKNET_LOG("重发包:%d;time=%u, dt=%u\n", (uint32_t)internalPacket->reliableMessageNumber, time, dt);
					...
					if (dt < (((CCTimeType)-1)/2) )
					{
						...
						//	internalPacket->dataBitLength	11456 = 1432*8	

						//congestionManager.GetMTU()	1464	//1492-28 = 1464
						//-DatagramHeaderFormat::GetDataHeaderByteLength()	9	unsigned int
						// GetMaxDatagramSizeExcludingMessageHeaderBits = 1455
...
// 							RAKNET_LOG("RESEND reliableMessageNumber %i with datagram %i\n", internalPacket->reliableMessageNumber.val, congestionManager.GetNextDatagramSequenceNumber().val);
						...
						//zz: 当且仅当retransmissionTime非常大？ 这也是一种可能??
						...
						// Removeme
#ifdef RAKNET_LOG_ZZ
//						xinfo2(TSF"Resend:%_; Bits()=%_;retransmissionTime=%_,dt=%_\n", (uint32_t)internalPacket->reliableMessageNumber, GetMaxDatagramSizeExcludingMessageHeaderBits(), (uint32_t)internalPacket->retransmissionTime, (uint32_t)dt);
#endif
					}
					else
					{
//#ifdef RAKNET_LOG_ZZ
//						RAKNET_LOG("等待发送[%d]: time=%lld, nextActionTime=%lld", internalPacket->reliableMessageNumber ,time, internalPacket->nextActionTime);
//#endif
						// Filled one datagram.
				...
				}//end of IsResendQueueEmpty()

				if (pushedAnything==false)
					break;
			}//end of Keep filling datagrams 
		}
		else
		{
		...
		{
#ifdef RAKNET_LOG_ZZ
//			RAKNET_LOG("[%d]ReliabilityLayer::Update S+ [%d/%d]\n", timeMs, allDatagramSizesSoFar, transmissionBandwidth);
#endif
			allDatagramSizesSoFar=0;
...
//						RAKNET_LOG("unacknowledgedBytes: %d -> %d ;sendReliableMessageNumberIndex=%d\n", unPre, unacknowledgedBytes, (int)sendReliableMessageNumberIndex);
					...
					// Testing1
// 					if (internalPacket->reliability==RELIABLE_ORDERED || internalPacket->reliability==RELIABLE_ORDERED_WITH_ACK_RECEIPT)
// 						RAKNET_LOG("SEND reliableMessageNumber %i in datagram %i\n", internalPacket->reliableMessageNumber.val, congestionManager.GetNextDatagramSequenceNumber().val);
		...
		for (unsigned int datagramIndex=0; datagramIndex < packetsToSendThisUpdateDatagramBoundaries.Size(); datagramIndex++)
		{
			...
			dhf.datagramNumber=congestionManager.GetAndIncrementNextDatagramSequenceNumber();	//设置数据报文++
...
//#ifdef RAKNET_LOG_ZZ
//			RAKNET_LOG("	[%d/%d]%p pushing datagram %i ; msgIndex=%d,msgTerm=%d\n", datagramIndex, packetsToSendThisUpdateDatagramBoundaries.Size(), this, dhf.datagramNumber.val, msgIndex, msgTerm);
//#endif
...
static int ncount_SendBitStream = 0;
//-------------------------------------------------------------------------------------------------------
...
void ReliabilityLayer::SendBitStream( RakNetSocket2 *s, SystemAddress &systemAddress, RakNet::BitStream *bitStream, RakNetRandom *rnr, CCTimeType currentTime)
{
	...
	//RAKNET_LOG("        [%d]ReliabilityLayer:SendBitStream(length=%d;count=%d) to %x\n", (unsigned int)(currentTime / 1000), length, ncount_SendBitStream++, systemAddress.address.addr4);
...
// 			RAKNET_LOG("%i ", internalPacket->reliableMessageNumber.val);
...
// 	}
	...
	{
	//	ValidateResendList();
		resendBuffer[messageNumber & RESEND_BUFFER_ARRAY_MASK]=0;	//zz: 每次确认一个，立即清理掉，避免
...
void ReliabilityLayer::SendAcknowledgementPacket( const DatagramSequenceNumberType messageNumber, CCTimeType time )
{
	...
	// REMOVEME
#ifdef RAKNET_LOG_ZZ
//	RAKNET_LOG("[T%d]SendAcknowledgementPacket(messageNumber=%d) %p Send ack %i \n", (unsigned int)(time / 1000), (unsigned int)messageNumber, this, messageNumber.val);
#endif
	nextAckTimeToSend=time;
	acknowlegements.Insert(messageNumber);
//#ifdef RAKNET_LOG_ZZ
//	RAKNET_LOG("ACK_DG:%i AckPush %i ", messageNumber.val,messageNumber);
//#endif
//	CC_DEBUG_PRINTF_2("", );
}
...
{
	//RAKNET_LOG("ReliabilityLayer::CreateInternalPacketFromBitStream(bitStream.GetNumberOfUnreadBits=%d)\n", bitStream->GetNumberOfUnreadBits());
...
void ReliabilityLayer::set_receivedPacketsBaseIndex(DatagramSequenceNumberType index)
{
	receivedPacketsBaseIndex = index;
}

void ReliabilityLayer::init_test(DatagramSequenceNumberType index)
{
	if (resetReceivedPackets)
	{
		hasReceivedPacketQueue.ClearAndForceAllocation(DEFAULT_HAS_RECEIVED_PACKET_QUEUE_SIZE, _FILE_AND_LINE_);
	...
	set_receivedPacketsBaseIndex(index);

	orderedReadIndex[0] = index;
}
//-------------------------------------------------------------------------------------------------------
// Get the SHA1 code
//-------------------------------------------------------------------------------------------------------
...
void ReliabilityLayer::InsertIntoSplitPacketList( InternalPacket * internalPacket, CCTimeType time )
{
		...
		(splitPacketChannelList[index]->splitPacketList.AddedPacketsCount()%splitMessageProgressInterval)==0)
	{
		...
		memcpy(progressIndicator->data+sizeof(MessageID)+sizeof(unsigned int)*3, splitPacketChannelList[index]->firstPacket->data, (size_t) BITS_TO_BYTES(splitPacketChannelList[index]->firstPacket->dataBitLength));
		outputQueue_Push(outputQueue,progressIndicator, __FILE__, __LINE__ );
	}
...
//#ifdef RAKNET_LOG_ZZ
//		 RAKNET_LOG("       RemoveFromList: -unacknowledgedBytes:%i \n", unacknowledgedBytes);
//#endif
...
void ReliabilityLayer::AddToListTail(InternalPacket *internalPacket, bool modifyUnacknowledgedBytes)
{
	if (modifyUnacknowledgedBytes)
	{
		unacknowledgedBytes+=BITS_TO_BYTES(internalPacket->headerLength+internalPacket->dataBitLength);
//#ifdef RAKNET_LOG_ZZ
//		 RAKNET_LOG("+unacknowledgedBytes:%i ", unacknowledgedBytes);
//#endif
	}
...
void ReliabilityLayer::SendACKs(RakNetSocket2 *s, SystemAddress &systemAddress, CCTimeType time, RakNetRandom *rnr, BitStream &updateBitStream)
{
	...
	{
		//RAKNET_LOG("[%d]ReliabilityLayer:SendACKs(Size=%d)\n", (unsigned int)(time / 1000), acknowlegements.Size());
...
//#ifdef RAKNET_LOG_ZZ
//	 RAKNET_LOG("	%p Pushed empty DatagramHistoryNode to datagram history at index %i\n", this, datagramHistory.Size()-1);
//#endif
}
...
#ifdef RAKNET_LOG_ZZ
	 RAKNET_LOG("       %p Pushed message %i to DatagramHistoryNode to datagram history at index %i\n", this, messageNumber.val, datagramHistory.Size()-1);
#endif
	return mnm;
}
