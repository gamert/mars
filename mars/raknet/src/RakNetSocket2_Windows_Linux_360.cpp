...
//            static int index = 0;
//            char buf[1024];
//            sprintf(buf, "Proxy%d", ++index);
//            FILE *fp = fopen(buf, "wb");
//            if (fp != NULL)
//            {
//                fwrite(sendParameters->data, 1, sendParameters->length, fp);
//                fclose(fp);
//            }
		...
		if (len<0)
		{
			RAKNET_DEBUG_PRINTF("sendto failed with code %i for char %i and length %i, error code '%s'.\n", len, sendParameters->data[0], sendParameters->length, strerror(errno));
		}
